...
const sendVerificationEmail = async (user, transaction_id) => {
  try {
    ...
    console.log(`Sending email from: ${process.env.TEST_EMAIL} to: ${process.env.ADMIN_EMAIL}`);
...
const sendAcceptanceEmail = async (user) => {
    try {
      ...
      console.log(`Sending email from: ${process.env.TEST_EMAIL} to: ${user.email}`);
      await transporter.sendMail(mailOptions); 
    } catch (error) {
...
export { sendVerificationEmail,sendAcceptanceEmail,sendRejectionEmail };
